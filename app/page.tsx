  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-6 font-sans bg-slate-50 min-h-screen pb-20">
      <header className="text-center py-4">
        ...
        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-1">Smart Shopping Assistant</p>
      </header>
      
      {/* ส่วนกรอกข้อมูลแบบ 2 Column */}
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className={`p-4 rounded-[2rem] shadow-lg border-4 transition-all duration-300 ${isAWinner ? 'border-green-500 bg-white scale-105 z-10' : 'border-transparent bg-white/60'}`}>
          <div className="flex justify-between items-center mb-3">
            ...
            {isAWinner && <span className="bg-green-500 text-white text-[8px] px-2 py-1 rounded-full font-bold">คุ้ม!</span>}
          </div>
          <div className="space-y-2">
            <input type="number" placeholder="ราคา" className="w-full p-3 bg-slate-100 rounded-xl font-bold text-sm outline-none focus:ring-2 ring-blue-500"
          ...
          <button onClick={() => handleSave('A')} className="w-full mt-3 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase active:scale-95 transition-all shadow-md">บันทึก</button>
          <div className="mt-4 pt-3 border-t border-dashed border-slate-100 text-center">
            <p className="text-[8px] text-slate-400 font-black uppercase mb-1">ราคาต่อ 1kg / 1L</p>
            <p className="text-xl font-black text-blue-900 leading-none">฿{formatDisplayPrice(unitPriceA)}</p>
          </div>
        </div>

        {/* Card แบบ B */}
        <div className={`p-4 rounded-[2.5rem] shadow-lg border-4 transition-all duration-500 ${isBWinner ? 'border-green-500 bg-white scale-105 z-10' : 'border-transparent bg-white/60'}`}>
          <div className="flex justify-between items-center mb-3">
            <span className={`text-lg font-black ${isBWinner ? 'text-green-600' : 'text-slate-400'}`}>แบบ B</span>
            {isBWinner && <span className="bg-green-500 text-white text-[8px] px-2 py-1 rounded-full font-bold">คุ้ม!</span>}
          </div>
          <div className="space-y-2">
            <input type="number" placeholder="ราคา" className="w-full p-3 bg-slate-100 rounded-xl font-bold text-sm outline-none focus:ring-2 ring-blue-500"
          ...
          <button onClick={() => handleSave('B')} className="w-full mt-3 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase active:scale-95 transition-all shadow-md">บันทึก</button>
          <div className="mt-4 pt-3 border-t border-dashed border-slate-100 text-center">
            <p className="text-[8px] text-slate-400 font-black uppercase mb-1">ราคาต่อ 1kg / 1L</p>
      ...
      <div className="text-center p-4 bg-white rounded-[2rem] shadow-sm border border-slate-100">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">บทสรุป</p>
        <p className={`text-sm font-bold ${isAWinner || isBWinner ? 'text-slate-700' : 'text-slate-400'}`}>
          {isAWinner ? '✅ แบบ A คุ้มค่ากว่าเมื่อเทียบต่อหน่วย' : isBWinner ? '✅ แบบ B คุ้มค่ากว่าเมื่อเทียบต่อหน่วย' : 'กรุณากรอกข้อมูลเพื่อเปรียบเทียบ'}
        </p>
      </div>
            ...
            history.map((item) => (
              <div key={item.id} className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex justify-between items-center group">
                <div>
                  ...
                  <p className="text-[9px] text-slate-400">{item.price} บาท / {item.quantity}{item.unit}</p>
                </div>
